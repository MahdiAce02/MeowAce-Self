...
def ensure_environment_directories() -> None:
    """Make sure required runtime folders exist on disk before file IO and set secure permissions."""
...
def load_bot_config() -> dict:
    ...
    try:
        ...
        print(f"[!] Critical Error: Could not read '{BOT_CONFIG_FILE}': {config_err}")
        sys.exit(1)


def save_bot_config(config_payload: dict) -> None:
    """Save bot configuration to bot_config.json."""
    ensure_environment_directories()
    tmp_path = BOT_CONFIG_FILE.with_suffix(".tmp")
    try:
        with open(tmp_path, "w", encoding="utf-8") as file_stream:
            json.dump(config_payload, file_stream, ensure_ascii=False, indent=2)
            file_stream.flush()
            os.fsync(file_stream.fileno())
        os.replace(tmp_path, BOT_CONFIG_FILE)
    except Exception as save_err:
        print(f"[!] Failed to save '{BOT_CONFIG_FILE}': {save_err}")
        if tmp_path.exists():
            try: tmp_path.unlink()
            except Exception: pass


_bot_data_lock = asyncio.Lock()

def get_bot_data_lock() -> asyncio.Lock:
    """Return the global async lock for bot_data mutations."""
    return _bot_data_lock
...
def save_bot_data(data_payload: dict) -> None:
    """Safely and atomically persist multi-user database to disk."""
    ensure_environment_directories()
    tmp_path = BOT_DATA_FILE.with_suffix(".tmp")
    try:
        with open(tmp_path, "w", encoding="utf-8") as file_stream:
            json.dump(data_payload, file_stream, ensure_ascii=False, indent=2)
            file_stream.flush()
            os.fsync(file_stream.fileno())
        os.replace(tmp_path, BOT_DATA_FILE)
    except Exception as save_err:
...
def attach_selfbot_handlers(client: TelegramClient, user_id: int) -> None:
    ...
    # Intercept outgoing custom aliases with recursion loop protection
    _dispatched_alias_ids = set()

    @client.on(events.NewMessage(outgoing=True))
    async def _alias_interceptor(event):
        # Ignore messages generated by alias execution to prevent infinite recursion
        if event.id in _dispatched_alias_ids:
            _dispatched_alias_ids.discard(event.id)
            return
        ...
        if matched_alias and target_commands:
            for cmd_str in target_commands:
                try:
                    sent = await client.send_message(event.chat_id, cmd_str)
                    if sent:
